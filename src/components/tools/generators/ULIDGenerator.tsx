...
import { generateUlid, decodeUlidTimestamp } from '../../../lib/ulid';
...
export default function ULIDGenerator() {
  ...
  const generate = useCallback(() => {
    setUlids(Array.from({ length: Math.min(quantity, 100) }, () => generateUlid()));
  }, [quantity]);
          ...
          {ulids.length === 0 ? (
            <div className="flex items-center justify-center h-full">
              <div className="text-center">
                <p className="text-sm text-neutral-700">Click Generate to produce ULIDs</p>
                <p className="text-[10px] text-neutral-800 mt-1">26 chars · Crockford base32</p>
              </div>
            </div>
          ) : (
            <ul className="divide-y divide-white/5">
              {ulids.map((ulid, i) => (
                <li key={i} className="group flex items-center gap-3 px-5 py-2.5 hover:bg-white/[0.03] transition-all">
                  ...
                  <span className="flex-1 font-mono text-sm text-neutral-300 select-all">{ulid}</span>
                  <span className="shrink-0 text-[10px] text-neutral-700 font-mono">
                    {new Date(decodeUlidTimestamp(ulid)).toISOString()}
                  </span>
                  <button
                    type="button"
                    onClick={() => copyOne(i, ulid)}
                    className={[
