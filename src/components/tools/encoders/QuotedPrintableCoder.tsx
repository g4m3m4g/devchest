...
export default function QuotedPrintableCoder() {
  ...
  const { output, error } = useMemo(() => {
    ...
    try {
      if (mode === 'encode') {
        return { output: encodeQuotedPrintable(input), error: null };
      }
      return { output: decodeQuotedPrintable(input), error: null };
    } catch (e) {
      return {
        output: '',
  ...
  return (
    <ToolLayout
      ...
      description="Convert text to Quoted-Printable (RFC 2045) encoding and back, with soft line wrapping."
      outputValue={output}
      onClear={() => setInput('')}
    ...
    >
      <div className="flex flex-col gap-4 h-full">
        ...
        <div className="grid grid-cols-2 gap-4 flex-1 min-h-0">
          <Panel label={mode === 'encode' ? 'Raw Text' : 'Quoted-Printable Text'}>
            <textarea
              value={input}
              onChange={e => setInput(e.target.value)}
              placeholder={mode === 'encode' ? 'Enter text to encode…' : 'caf=C3=A9'}
              spellCheck={false}
          ...
          <Panel label={mode === 'encode' ? 'Quoted-Printable Output' : 'Decoded Text'}>
            {error ? (
