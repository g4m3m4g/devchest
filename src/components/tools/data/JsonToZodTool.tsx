...
export default function JsonToZodTool() {
  ...
  const result = useMemo(
    () => generateZodSchema(input, rootName.trim() || 'Root'),
    [input, rootName],
  );
        ...
        >
          {result.error ? (
            ...
            <pre className="flex-1 w-full min-h-0 overflow-auto px-4 py-3 font-mono text-sm text-neutral-300 leading-relaxed whitespace-pre">
              {result.output || <span className="text-neutral-800">Generated schema will appear here…</span>}
            </pre>
          )}
        </Panel>
      </div>
